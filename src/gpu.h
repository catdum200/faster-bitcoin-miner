...
typedef struct {
    ...
    unsigned vr_iters;      /* nonces per work-item per launch, version layout */
    double launch_hashes;   /* target hashes per kernel launch (keeps launches short) */
...
/* Run-time tuning: iters, launch_hashes and hit_cap (up to the capacity
 * allocated at open) may be changed between scans; the rest are fixed. */
