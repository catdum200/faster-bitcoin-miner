...
| 9 | **[minor]** G3's reasoning is partly wrong, though its choice is right; G2 pays off from 32 versions | **Accepted.** New rationale: the SALU has no 32-bit rotate, portable OpenCL puts wave-uniform rotates on the VALU (measured 0.96x of G1), and `v_readlane` costs VALU and occupancy (1.14x). The audit checks "no VMEM in the loop except the hit path, SMEM ≤ 8", not a load shape. The pre-pass costs 463 VALU per row spread over the versions sharing it: 0.7% even for one wave (32) of versions |
